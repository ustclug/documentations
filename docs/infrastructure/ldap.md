...
!!! ansible "Ansible: `sudo-migration.yml`"

    我们的 [infra](automation.md) 仓库中含有 `sudo-migration.yml` Ansible playbook，可以帮助迁移 Debian 上的 `sudo-ldap` 到 `sssd`。

    **注意**：该 playbook 不包含为 Debian bookworm 修改 AppArmor 配置的步骤。
